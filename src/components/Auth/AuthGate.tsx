import { useEffect } from 'react'
import { useGoogleLogin } from '@react-oauth/google'
import styles from './AuthGate.module.css'

const SCOPE = 'https://www.googleapis.com/auth/spreadsheets'

interface AuthGateProps {
  onLogin: (token: string, expiresIn: number) => void
}

function AuthGate({ onLogin }: AuthGateProps) {
  const login = useGoogleLogin({
    onSuccess: (r) => onLogin(r.access_token, r.expires_in),
    onError: (e) => console.error('Login failed:', e),
    scope: SCOPE,
  })

  // Attempt a silent re-auth on mount — succeeds if the user has an active
  // Google session and has already granted the required scope.
  const loginSilently = useGoogleLogin({
    onSuccess: (r) => onLogin(r.access_token, r.expires_in),
    onError: () => { /* silently show the login button */ },
    scope: SCOPE,
    prompt: 'none',
  })

  useEffect(() => {
    loginSilently()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [])

  return (
    <div className={styles.container}>
      <div className={styles.card}>
        <div className={styles.logoMark}>
          <img src="/eKakeibo-logo.png" alt="eKakeibo" className={styles.logoImage} />
        </div>
        <h1 className={styles.title}>eKakeibo</h1>
        <p className={styles.tagline}>Mindful finances</p>
        <p className={styles.subtitle}>
          Your personal finance dashboard.<br />
          Connect your Google Sheet to get started.
        </p>
        <button className={styles.loginButton} onClick={() => login()}>
          <svg width="20" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="currentColor" opacity="0.8" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="currentColor" opacity="0.8" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="currentColor" opacity="0.8" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="currentColor" opacity="0.8" />
          </svg>
          Connect Google Account
        </button>
        <p className={styles.disclaimer}>
          Read and write access to your spreadsheet.
        </p>
      </div>
    </div>
  )
}

export default AuthGate
